...
| **Insertion at Tail** (`push_back`) | $O(N)$ | $O(N)$ | Requires traversing to the end of the list unless a tail pointer is maintained. |
...
| **Deletion at Tail / Index** (`delete_at`) | $O(N)$ | $O(N)$ | Traverses to the target index to unlink and free the node. |
